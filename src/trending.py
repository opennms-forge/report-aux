...
def time_trend(parsed_metrics: dict, interface: str, label: list) -> dict:  # noqa: C901
                ...
                for metric in label:
                    if "out" in metric.lower():
                        if metric in parsed_metrics[interface]["day_of_week"][j][k]:
                            ...
                            if k == "total":
                                trend["y"].append("average")
                            else:
                                trend["y"].append(f"{k}:00")
                            if (
                                parsed_metrics[interface]["day_of_week"][j][k][metric]
                                is not None
                            ):
                                trend["z"].append(
                                    parsed_metrics[interface]["day_of_week"][j][k][
                                        metric
                                    ]
                                    or 0
                                )
    ...
    for k in parsed_metrics[interface]["hour_of_day"]:
        ...
        for metric in label:
            if "out" in metric.lower():
                trend["hour"][k]["Bytes Out"].append(
                    parsed_metrics[interface]["hour_of_day"][k].get(metric) or 0
                )
            elif "in" in metric.lower():
                trend["hour"][k]["Bytes In"].append(
                    parsed_metrics[interface]["hour_of_day"][k].get(metric) or 0
                )
    for k in parsed_metrics[interface]["hour_of_day"]:
                ...
                for metric in label:
                    if "out" in metric.lower():
                        if parsed_metrics[i]["ts"][j].get(metric) not in [None, [None]]:
                            stats_out["y"].append(
                                parsed_metrics[i]["ts"][j].get(metric) * -1
                            )
                        else:
...
def get_trend_graph(trend: dict, margin: int = 80) -> px.scatter:
    ...
    fig = px.scatter(
        ...
        size=trend["z"],
        color=trend["c"],
        labels={"x": "Day of Week", "y": "Time", "size": "Bytes", "color": "Metric"},
        color_discrete_map={"Bytes Out": theme["out"], "Bytes In": theme["in"]},
    )
    fig.add_shape(
        y0="8:00",
        y1="8:00",
    ...
    fig.add_vrect(
        x0=4.5,
        x1=6.5,
        fillcolor=theme["weekend"],
        opacity=0.25,
        layer="below",
        line_width=0,
    )
    fig.layout.margin.l = margin  # noqa E741
...
def get_trend_line(
    stats_out: dict, stats_in: dict, weekends: dict, margin: int = 80
) -> go.Figure:
    ...
    fig2.add_trace(
        go.Scatter(
            ...
            mode="lines",
            name="BytesOut",
            line={"color": theme["out"]},
            fill="tozeroy",
            fillcolor=theme["outfill"],
        )
    )
    fig2.add_trace(
        go.Scatter(
    ...
    fig2.layout.margin.l = margin  # noqa E741
